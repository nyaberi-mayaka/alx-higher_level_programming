...
def matrix_mul(m_a, m_b):
    ...
    if not all([[isinstance(num, int) for num in item] for item in m_a]) and\
       not all([[isinstance(num, float) for num in item] for item in m_a]):
        raise TypeError("m_a should contain only integers or floats")

    if not all([[isinstance(num, int) for num in item] for item in m_b]) and\
       not all([[isinstance(num, float) for num in item] for item in m_b]):
